    ...
    with tempfile.TemporaryDirectory() as temp: 
        ...
        for dirpath, dirs, files in sorted(os.walk(temp)):  
            for d in dirs:
                ...
                if i == 0: 
                    df = pd.read_csv(filepath_or_buffer=quantFilePath, sep="\t", index_col=0, usecols=["Name", "TPM"]) 
                    df = df.rename(columns={"TPM": d}) 
                #join other files onto the first dataframe 
                else:
                    tempdf = pd.read_csv(filepath_or_buffer=quantFilePath, sep = "\t", index_col=0, usecols=["Name", "TPM"]) 
